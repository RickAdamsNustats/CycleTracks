...
public class TripUploader extends AsyncTask <Long, Integer, Boolean> {
    ...
    final String postUrl = "http://pacelogger.nustats.com/savetrip_android.asp";
    ...
    protected void onPreExecute() {
        Toast.makeText(mCtx.getApplicationContext(),"Submitting trip.  Thanks for using PaceLogger!", Toast.LENGTH_LONG).show();
    }

    @Override
    protected void onPostExecute(Boolean result) {
        try {
        	//comment out the if when it breaks.
        	if (result)
        	{
           ...
           {
                Toast.makeText(mCtx.getApplicationContext(),"PaceLogger couldn't upload the trip, and will retry when your next trip is completed.", Toast.LENGTH_LONG).show();
           }
        } catch (Exception e) {
